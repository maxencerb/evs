...
const script = evscript({ name: 'totalSupplyOf', args: [t.address] }, (s, token) => {
  const supply = s.read({ address: token, abi: erc20, functionName: 'totalSupply' });
...
Four overloads, two modes. Every result spreads directly into viem's `readContract`:
...
| `toViem({ mode: 'stateOverride', sender })` | `{ abi, address: sender, stateOverride: [{ address: sender, code: runtimeBytecode }], account: sender }` — [sender mode](/guides/execution/#sender-mode): sub-call targets see `msg.sender = sender` |
...
| `EvsDecodeError(uint256 site)` | `0x20cf27b7` | A strict `s.read` got returndata that fails to decode; `site` identifies the recorded call site |
...
The bytecode itself is lowered from `eliminateDeadCode(script.ir)` — the pass `compile()`
runs on every compile (see [How it works](/concepts/how-it-works/#stage-3-dead-code-elimination)).
It is exported (alias `dce`) so you can inspect exactly what was compiled: same tables, same
site ids, dead statements gone, and a fixpoint — applying it twice changes nothing.

```ts
import { compile, eliminateDeadCode, evscript, serializeIr, t } from '@maxencerb/evs';

const script = evscript({ name: 'guarded', args: [t.uint256, t.uint256] }, (s, a, b) => {
  const unused = a.mul(b); // never read — dropped, its overflow check with it
  void unused;
  return s.return({ sum: a.add(b) });
});

const compiled = compile(script);
const lowered = eliminateDeadCode(compiled.ir); // what the bytecode was lowered from
lowered.values === compiled.ir.values; // true — the value table is never renumbered
serializeIr(eliminateDeadCode(lowered)) === serializeIr(lowered); // true — idempotent
```

Stability notes:

- The format is versioned: `irVersion: 1`. `deserializeIr` rejects any other version, and the
  compiler emits only version 1.
...
artifact was built with — defaults applied: `evmVersion: 'cancun'`, `optimize: false`,
`locations: true`, identity `peephole`, no-op `onDiagnostic`. See [evscript](/reference/evscript/) for the option-by-option
