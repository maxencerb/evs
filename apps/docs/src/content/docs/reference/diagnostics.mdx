...
| `UNSUPPORTED_V0` | `EvsTypeError` | A feature evs does not support yet: a genuinely ambiguous overloaded call (several overloads accept the given args — type the literal with `s.lit` or prune the ABI) or `t.fromOutputs` on an overloaded name whose overloads differ in outputs. Every array/tuple shape — `tuple[][]`, any nesting depth, fixed-size `T[N]` — is supported since [#4](https://github.com/maxencerb/evs/issues/4) | Recording |
| `ABI_SHAPE` | `EvsTypeError` | Malformed ABI material: invalid struct field names, duplicate or invalid return component names, or a malformed ABI fragment passed to `s.read`. (An invalid script *name* given to `evscript()` is caught at the entry point as `TYPE_MISMATCH`.) | Recording |
...
| --- | --- | --- |
| `LOOP_ALLOCATION` | An allocating statement sits inside a loop body or header: `s.newArray`, `s.tuple(...)` (struct/tuple flat-block allocation), an `s.read`/`s.tryRead` with outputs (each call snapshots returndata), a dynamic literal materialization, or a call to an `s.fn` whose body transitively allocates | evs never resets the free memory pointer, so memory — and memory-expansion gas — grows monotonically with every iteration for the lifetime of the call |
