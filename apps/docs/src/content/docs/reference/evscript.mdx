...
- Every arg type must be a valid `EvsType` — a word/dynamic type, a string array, or a
  `TupleType` (struct/tuple). Shapes outside the supported set throw with code `UNSUPPORTED_V0`.
