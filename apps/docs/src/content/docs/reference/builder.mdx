...
const mkPos = evscript({ name: 'mkPos', args: [t.address] }, (s, owner) => {
  ...
  return s.return({ liq: pos.liquidity.get(), pos: pos.expr() }); // a field, and the whole struct
});
```
...
- Every value must be an `Expr` — a struct/tuple value is passed as its `Tuple` handle's
  `.expr()`, which nests as a `'tuple'` component. Type bare literals with `s.lit` first.
...
(literal `number` index); and `expr()` is the raw `Expr<C>` memref — what you return or pass on.
