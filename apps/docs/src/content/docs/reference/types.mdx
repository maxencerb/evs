...
| `ArgType`     | The script-argument value type — an alias for `EvsType` (word, dynamic, string array, or `TupleType`); types `ScriptIr.args` and `ArgSpec.type`. `s.fn` params are narrower (string-encoded only) — that bound lives on [`arg()`](/reference/evscript/#arg-and-argspec-for-sfn), not here. |
...
`.at(i)`, `s.newArray` — and the same reference semantics; an `s.call` `tuple[]` output is an
