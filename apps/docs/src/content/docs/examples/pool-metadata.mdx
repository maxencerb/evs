...
`s.call` types like viem's `readContract`: the inline `as const` pool ABI and viem's
...
return a readonly tuple of `Expr`s, so `slot0[1]` is the tick. See
[calls](/guides/calls/) for the full `s.call` surface and
[values and types](/guides/values-and-types/) for how `Expr` handles work.
...
mode, block pinning, and the `s.env` caveats, see [execution](/guides/execution/).
