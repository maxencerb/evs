...
`account` call parameter — it is the only mode where `s.env('caller')` is meaningful (and its
`sender` option runs the script *as* an address, so sub-call targets see that `msg.sender` too —
see [sender mode](/guides/execution/#sender-mode)). Pinning
