...
1. **Define the script.** `evscript` takes a header — `{ name, args }`, with each argument
   declared via `arg(name, type)` from the `t` namespace — and a builder callback. The
   callback receives the builder `s`, reads script arguments from `s.args`, records calls
...
- [Writing scripts](/guides/writing-scripts/) — the script header, `s.args`, and the recording-time vs run-time model
