...
| Class             | Thrown for                                                                                                   |
| ----------------- | ------------------------------------------------------------------------------------------------------------ |
| `EvsTypeError`    | type mismatches, literal range violations, overloaded ABI names, unsupported types, folds that would certainly panic |
...
— transport failures never masquerade as script errors. The error tree is inspected by shape,
not by class, so a revert thrown by a different copy of viem than the one evs resolved (a
second bundle, a pnpm-duplicated version) still decodes.
