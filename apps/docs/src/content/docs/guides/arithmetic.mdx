...
`eq`/`neq` are restricted to word types — memref (`string`/`bytes`/`T[]`) equality is not supported ([#38](https://github.com/maxencerb/evs/issues/38)); compare hashes via `s.keccak256` instead. Boolean `and`/`or` are **eager**, not short-circuiting: both sides are already-computed values by the time you combine them. For conditional execution, use `s.if` ([control flow](/guides/control-flow/)).
