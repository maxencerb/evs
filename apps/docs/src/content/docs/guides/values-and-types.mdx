...
Not supported yet ([#4](https://github.com/maxencerb/evs/issues/4); `UNSUPPORTED_V0` at recording): two-level tuple arrays (`tuple[][]`), arrays nested deeper than `[][]`, and fixed-size `T[N]`.
