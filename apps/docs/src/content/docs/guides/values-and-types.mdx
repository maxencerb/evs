...
A composite-array **literal** (a plain JS array of struct objects, of arrays, or of strings) is constructible anywhere the value type is expected — it is built at record time into a fresh `[length][p0…]` block with reference semantics (not a flat data segment):

```ts
import { evscript, t } from '@maxencerb/evs';

const literals = evscript({ name: 'literals', args: [] }, (s) => {
  const positions = s.lit(t.array(t.struct({ liquidity: t.uint128, owner: t.address })), [
    { liquidity: 100n, owner: '0x0000000000000000000000000000000000000001' },
    { liquidity: 200n, owner: '0x0000000000000000000000000000000000000002' },
  ]);
  const grid = s.lit(t.array(t.array(t.uint256)), [[1n, 2n], [3n]]); // uint256[][]
  const tags = s.lit(t.array(t.string), ['alpha', 'beta']); // string[]
  return s.return({ positions, grid, tags });
});
```
