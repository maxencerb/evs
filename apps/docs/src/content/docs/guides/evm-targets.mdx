...
```ts
import { compile, evscript, t, type EvmVersion } from '@maxencerb/evs';
...
- `MCOPY` is EIP-5656, activated in Cancun. On older targets memory copies go through a shared
  `@memcpy` word-loop subroutine, emitted once per pre-Cancun build (a few dozen bytes, present
  whether or not the script actually copies memory).
