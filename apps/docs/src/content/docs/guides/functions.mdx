...
- **Parameters** are declared with the same `arg(name, type)` specs as script arguments, and
  may be any `EvsType` — dynamic types (`string`, `bytes`, arrays) pass as memref pointer
  words. The body receives them as positionally typed `Expr`s.
- **Returns** can be a single `Expr`, a readonly tuple of `Expr`s (use `as const`, as above),
  or `void`.
- **Call sites** accept literals anywhere a parameter type allows them — `minMax(s.args.lo, 100n)`
