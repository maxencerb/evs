...
jobs:
  publish:
    ...
    steps:
      ...
      - name: Re-sync lockfile after version bump
        # NOT frozen: the bump changed the workspace version, and packing against a stale
        # bun.lock makes `bun pm pack --quiet` chatty (extra "Saved lockfile" line broke the
        # GITHUB_OUTPUT capture in run 27402030447) and risks oven-sh/bun#20477 stale
        # version substitution. Dependencies are unchanged; only the lock's workspace
        # version is updated.
        run: bun install
      ...
      - name: Pack (bun rewrites workspace:/catalog: protocols)
        id: pack
        working-directory: packages/evs
        # tail -n1: pack's stdout must be exactly the filename for GITHUB_OUTPUT
        run: echo "tarball=$(bun pm pack --quiet | tail -n1)" >> "$GITHUB_OUTPUT"
